...
@section('content')
        ...
        @php 
            $plants = App\Models\Package::where('type', 'plant')->where('status', 'open')->get();
        @endphp
        @if($plants->count() >= 1)
        <section class="bg-[#F3FFF9]">
            ...
            <div class="wrapper mt-20">
              <div class="carousel owl-carousel p-3">
                    ...
                    @php 
                        $category = App\Models\Category::where('id', $plant->category_id)->first();
                    @endphp
                      <div :class="card card-{{ $plant->id }} !self-start lg:w-1/3 relative">
                        @if($plant->image == null)
                        <img class="rounded-3xl" src="/static-assets/plant-estate-hero-image.2b800cfc.png" alt="">
                        @else
                        <img class="rounded-3xl" src="{{ $plant->image }}" alt="" style="width: 100%; height: 260px; object-fit: cover;">
                        @endif
                        <div class="px-5 py-5 bg-white -translate-y-10 rounded-xl shadow-lg lg:w-[90%]">
                          <h3 class="text-ink text-lg py-2 font-bold">{{ $plant->name }}</h3>
                          ...
                          <a href="{{ route('packages.show', ['package' => $plant['id']]) }}"
        ...
        <section class="section-container">
            <div
                ...
                <div class="self-end w-full lg:w-2/5 lg:pb-20">
                    ...
                    <a href="{{ route('investments', 'plant') }}"
